    ...
    const insertTrain = () =>{
        try{
         ...
         if(TRAIN_NUMBER.length == 0 || TRAIN_NAME.length == 0 || SOURCE_LOCATION.length == 0 || DESTINATION_LOCATION.length == 0){
          ...
          Axios.post('http://localhost:1337/inserttrain', {TRAIN_NUMBER: TRAIN_NUMBER, TRAIN_NAME: TRAIN_NAME, ARRIVAL_TIME: ARRIVAL_TIME, DEPARTURE_TIME: DEPARTURE_TIME, SOURCE_LOCATION: SOURCE_LOCATION, DESTINATION_LOCATION: DESTINATION_LOCATION, AVAILABLE_SEATS: AVAILABLE_SEATS, BOOKED_SEATS: BOOKED_SEATS, ADMIN_KEY: ADMIN_KEY, TYPE: TYPE, STATION_NAME: STATION_NAME,})
          .then(()=>{
            alert("success");
          });
          reset();
        ...
        <div className='row'>
          <div className='col-12 text-center'>
            <div className='contactForm'>
                ...
                <button type="submit" className='submit-btn' onClick={insertTrain} disabled={disabled} type='submit'>
                  Add Train
                </button>
